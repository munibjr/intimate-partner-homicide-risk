...
class FeatureExtractor:
    ...
    def _count_violence(self, events: List[Dict]) -> float:
        """Count violence incidents."""
        return float(sum(1 for e in events if e.get('type') == 'police_report' and e.get('violence')))
    ...
    def _count_threats(self, events: List[Dict]) -> float:
        """Count threat events."""
        return float(sum(1 for e in events if e.get('type') == 'threat'))
    ...
    def _count_isolation(self, events: List[Dict]) -> float:
        """Count isolation/control behaviors."""
        return float(sum(1 for e in events if e.get('type') == 'isolation_behavior'))
    
    def _count_breaches(self, events: List[Dict]) -> float:
        """Count restraining order breaches."""
        return float(sum(1 for e in events if e.get('type') == 'restraining_order_breach'))
    ...
    def _days_since_incident(self, events: List[Dict]) -> float:
        ...
        return min(1.0, days / 365.0)  # Normalize to 0-1
    
    def _is_escalating(self, events: List[Dict]) -> bool:
        """Check if case is getting worse."""
        if len(events) < 2:
            return False
        # Simple heuristic: later events have higher severity
    ...
    def _escalation_velocity(self, events: List[Dict]) -> float:
        ...
        if days_span == 0:
            return 0.0
        return float(len(events)) / days_span
